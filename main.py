...
lecturer_df = pd.read_csv("db/lecturers.csv")

# Read the module data from the CSV file using Pandas
module_df = pd.read_csv("db/modules.csv")

# Convert the Pandas DataFrames to Python data structures
lecturer_data = lecturer_df.to_dict(orient="records")
module_data = module_df.to_dict(orient="records")
...
for module1 in module_data:
    for module2 in module_data:
        if (
